...
class RawDataRequest(db.Model):
    __tablename__ = "raw_data_requests"
    id = db.Column(db.Integer, primary_key=True)
    request_id = db.Column(db.Integer, db.ForeignKey("requests.id"), nullable=False)
    requester_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    researcher_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    status = db.Column(db.String(20), default="requested")  # requested / sent
    file_path = db.Column(db.String(500), default="")
    file_name = db.Column(db.String(200), default="")
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    sent_at = db.Column(db.DateTime, nullable=True)

    request = db.relationship("Request", backref="raw_data_requests")
    requester = db.relationship("User", foreign_keys=[requester_id])
    researcher = db.relationship("User", foreign_keys=[researcher_id])
